...
- [X] T001 Initialize the repository as a git working tree on the default branch `main` in `E:\w\auto-specify` (`git init -b main`) so the home can be committed and pushed to GitHub; add **no** `.gitignore`, `.gitattributes`, or other new root file (research D7, constitution V)
- [X] T002 Verify the local prerequisites in [quickstart.md](./quickstart.md#prerequisites): `pi --version` resolves the pi CLI, `node --version` is ≥ 22.6 (built-in type stripping for the existing checks), and no pipeline run is active in the session that performs the move (spec Assumption: install happens when no run is in progress)
...
- [X] T013 [US2] Install from the published home using the unpinned source only: run `pi install git:github.com/<owner>/<repo>` and confirm `pi list` shows the repository exactly once as a git source with **no** `@ref` suffix; no local path, no repository-root install, no hand-copied overlay (FR-004, FR-014, SC-005). Evidence 2026-09-15: `pi install git:github.com/omid3098/auto-specify` cloned to `~/.pi/agent/git/github.com/omid3098/auto-specify`; `pi list` shows `git:github.com/omid3098/auto-specify` once, no `@ref`.
- [X] T014 [US2] Restart pi and confirm the extension is loaded from the installed GitHub clone rather than this checkout: `/speckit-run`, `/speckit-cancel`, and `/speckit-answer` are available in the new session and no `.pi/extensions/speckit-orchestrator/` exists in the target project (FR-005, FR-006). Evidence 2026-09-15: fresh session in an unrelated project (`/tmp/pi-ext-check`), RPC `get_commands` returns all three commands with `source: extension`; no `.pi/extensions/` in the home or the target project.
...
- [X] [P] [US3] T022 Clone the published `<owner>/<repo>` into a temporary directory and run `node --test extensions/speckit-orchestrator/run.test.js` there to prove the checks ship with the home and pass without this working copy's local state (FR-011, quickstart V3). Evidence 2026-09-15: `git clone https://github.com/omid3098/auto-specify /tmp/as-clone` → `tests 12 / pass 12 / fail 0`.
...
- [X] T023 [P] Sweep the home for references that describe the extension's current location as `.pi/extensions/speckit-orchestrator/` and update the current-guidance ones (`README.md`, `.pi/prompts/*.md`) to `extensions/speckit-orchestrator/`; leave the historical feature artifacts under `specs/001-speckit-orchestrator/` unchanged as a record of the previous layout (FR-001, FR-013)
- [X] T024 [P] Remove the scratch `Sync Impact Report (scratch; remove before commit)` block at the top of `.specify/memory/constitution.md` so the published home carries no pre-commit scratch text (FR-009, constitution Governance)
- [X] T025 Simplicity guard (constitution V, research D1/D5/D8): confirm the rehome introduced no root `package.json`, no `pi` manifest, no version file, no npm publish configuration, no `.gitignore`, and no new settings or disable surface; if any appeared, remove it rather than documenting it
