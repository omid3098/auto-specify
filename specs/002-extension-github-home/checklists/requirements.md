...
- Publish-facts follow-up 2026-09-15: operator confirmed the concrete slug `omid3098/auto-specify` (folded into the Clarifications section, FR-014, and Assumptions), the MIT copyright holder `Omid Saadat` (folded into FR-015), running the install/update in pi (in scope), and manual verification of the installed-pi scenarios in a later session (recorded as an Assumption, not claimed passing). No `[NEEDS CLARIFICATION]` markers remain; all 16 checklist items still pass — no marker changes were required.
